...
import { Users, Search, Mail, RefreshCw, AlertCircle, Building2, X, CheckCircle, Phone, ClipboardList, Link as LinkIcon } from 'lucide-react';
...
const ContratistaModal: React.FC<{
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fn  = contratista ? apiPut : apiPost;
...
const ConfirmDelete: React.FC<{
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const res  = await apiDelete(`${API}/contratistas/${contratista.id}`);
      const data = await res.json();
...
const Contratistas: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [c, a] = await Promise.all([cRes.json(), aRes.json()]);
      if (c.success) { const sorted = [...c.data].sort((x: any, y: any) => new Date(y.fecha_creacion).getTime() - new Date(x.fecha_creacion).getTime()); setContratistas(sorted); setFiltered(sorted); }
              ...
              return (
                <div key={c.id} className={`card hover:shadow-lg transition-shadow ${cardColor}`}>
                          ...
                          {myAlcances.map(al => (
                            <div key={al.id} className="flex items-center justify-between bg-primary-50 border border-primary-100 rounded-md px-2.5 py-1.5">
                              ...
                              <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                                <span className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${PERIOD_COLOR[al.periodicidad] || 'bg-gray-100 text-gray-600'}`}>
                  ...
                  <div className="border-t border-gray-100 px-5 py-3">
                    <Link to="/actividades"
                      className="flex items-center justify-center gap-2 text-xs font-medium text-primary-600 hover:text-primary-800">
                      <LinkIcon className="h-3.5 w-3.5" />
                      Gestionar actividades asignadas
                    </Link>
                  </div>
                </div>
              );

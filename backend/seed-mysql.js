...
async function main() {
  ...
  // ── Metas ─────────────────────────────────────────────────────────────────
  const metasData = [
    { codigo: 'META-001', nombre: 'Construcción de Sede Corporativa',         descripcion: 'Construcción completa del edificio principal de la nueva sede corporativa, incluyendo estructura, acabados y urbanismo exterior.',                                                    estado: 'EN_PROGRESO', fecha_limite: '2025-12-31', creador_id: adminId },
    { codigo: 'META-002', nombre: 'Sistema Integral de Seguridad',            descripcion: 'Instalación de cámaras IP, control de acceso biométrico, sistema de alarmas y centro de monitoreo en todas las instalaciones.',                                                          estado: 'EN_PROGRESO', fecha_limite: '2025-09-30', creador_id: adminId },
    { codigo: 'META-003', nombre: 'Renovación y Mantenimiento de Infraestructura', descripcion: 'Renovación completa de fachadas, impermeabilización de techos, pintura general y mantenimiento de áreas comunes.',                                                                  estado: 'EN_PROGRESO', fecha_limite: '2025-11-30', creador_id: usuario2Id },
    { codigo: 'META-004', nombre: 'Modernización de Red Eléctrica',           descripcion: 'Sustitución del tablero eléctrico principal, instalación de UPS, planta eléctrica de emergencia y cableado estructurado.',                                                               estado: 'PENDIENTE',   fecha_limite: '2025-08-31', creador_id: adminId },
    { codigo: 'META-005', nombre: 'Adecuación de Espacios de Trabajo',        descripcion: 'Remodelación de oficinas, instalación de divisiones modulares, mobiliario ergonómico y adecuación de salas de reuniones.',                                                                estado: 'COMPLETADA',  fecha_limite: '2025-06-30', creador_id: usuario2Id },
  ];
  const metas = [];
  ...
  const avancesData = [
    // META-001 CONT-001
    { numavance: 1, porcentaje_avance: 20, descripcion: 'Excavación y preparación del terreno completada. Estudios de suelo realizados.', fecha_presentacion: '2025-02-15', metaId: M('META-001').id, contratistaId: C('CONT-001').id, alcanceId: AL('CONT-001','META-001'), reportado_por_id: adminId },
    { numavance: 2, porcentaje_avance: 45, descripcion: 'Cimentación completada al 100%. Vaciados 800 m³ de concreto en zapatas y vigas de fundación.', fecha_presentacion: '2025-03-31', metaId: M('META-001').id, contratistaId: C('CONT-001').id, alcanceId: AL('CONT-001','META-001'), reportado_por_id: adminId },
    { numavance: 3, porcentaje_avance: 60, descripcion: 'Estructura de concreto de pisos 1 al 4 completada. Encofrado y vaciado del piso 5 en proceso.', fecha_presentacion: '2025-04-30', metaId: M('META-001').id, contratistaId: C('CONT-001').id, alcanceId: AL('CONT-001','META-001'), reportado_por_id: adminId },
    { numavance: 4, porcentaje_avance: 85, descripcion: 'Obra gris de los 8 pisos concluida al 100%. Inicio de instalaciones en pisos 1 al 3.', fecha_presentacion: '2025-06-30', metaId: M('META-001').id, contratistaId: C('CONT-001').id, alcanceId: AL('CONT-001','META-001'), reportado_por_id: adminId },
    { numavance: 5, porcentaje_avance: 98, descripcion: 'Edificio prácticamente terminado. Acabados finales. Pruebas de instalaciones aprobadas.', fecha_presentacion: '2025-12-15', metaId: M('META-001').id, contratistaId: C('CONT-001').id, alcanceId: AL('CONT-001','META-001'), reportado_por_id: adminId },
    // META-002 CONT-006
    { numavance: 1, porcentaje_avance: 30, descripcion: 'Instalación de 40 cámaras IP en perímetro externo. Configuración inicial del DVR completada.', fecha_presentacion: '2025-03-10', metaId: M('META-002').id, contratistaId: C('CONT-006').id, alcanceId: AL('CONT-006','META-002'), reportado_por_id: adminId },
    { numavance: 2, porcentaje_avance: 65, descripcion: '80 cámaras interiores instaladas en pisos 1 al 4. Centro de monitoreo operativo al 70%.', fecha_presentacion: '2025-04-15', metaId: M('META-002').id, contratistaId: C('CONT-006').id, alcanceId: AL('CONT-006','META-002'), reportado_por_id: adminId },
    { numavance: 3, porcentaje_avance: 95, descripcion: '120 cámaras IP operativas. Centro de monitoreo equipado. Integración con sistema de alarma.', fecha_presentacion: '2025-06-30', metaId: M('META-002').id, contratistaId: C('CONT-006').id, alcanceId: AL('CONT-006','META-002'), reportado_por_id: adminId },
    // META-002 CONT-002
    { numavance: 1, porcentaje_avance: 50, descripcion: 'Lectores biométricos instalados en 8 accesos. Enrolamiento de 120 empleados completado.', fecha_presentacion: '2025-03-25', metaId: M('META-002').id, contratistaId: C('CONT-002').id, alcanceId: AL('CONT-002','META-002'), reportado_por_id: adminId },
    { numavance: 2, porcentaje_avance: 100, descripcion: 'Sistema de control de acceso biométrico 100% operativo. Capacitación finalizada.', fecha_presentacion: '2025-06-15', metaId: M('META-002').id, contratistaId: C('CONT-002').id, alcanceId: AL('CONT-002','META-002'), reportado_por_id: adminId },
    // META-003 CONT-005
    { numavance: 1, porcentaje_avance: 55, descripcion: 'Pintura de fachada norte y sur completada (3.200 m²). Preparación de fachadas este y oeste.', fecha_presentacion: '2025-04-20', metaId: M('META-003').id, contratistaId: C('CONT-005').id, alcanceId: AL('CONT-005','META-003'), reportado_por_id: adminId },
    { numavance: 2, porcentaje_avance: 90, descripcion: 'Fachadas externas al 100%. Interiores pisos 6 al 8 en proceso. Señalización completada.', fecha_presentacion: '2025-07-30', metaId: M('META-003').id, contratistaId: C('CONT-005').id, alcanceId: AL('CONT-005','META-003'), reportado_por_id: adminId },
    { numavance: 3, porcentaje_avance: 100, descripcion: 'Obra de pintura concluida en su totalidad. 18.000 m² intervenidos. Acta de entrega firmada.', fecha_presentacion: '2025-09-30', metaId: M('META-003').id, contratistaId: C('CONT-005').id, alcanceId: AL('CONT-005','META-003'), reportado_por_id: adminId },
    // META-004 CONT-004
    { numavance: 1, porcentaje_avance: 25, descripcion: 'Desmontaje del tablero antiguo. Instalación del nuevo tablero principal 400A iniciada.', fecha_presentacion: '2025-05-20', metaId: M('META-004').id, contratistaId: C('CONT-004').id, alcanceId: AL('CONT-004','META-004'), reportado_por_id: adminId },
    { numavance: 2, porcentaje_avance: 75, descripcion: 'Planta de emergencia 250 kVA instalada y sincronizada. Transfer automático configurado.', fecha_presentacion: '2025-08-10', metaId: M('META-004').id, contratistaId: C('CONT-004').id, alcanceId: AL('CONT-004','META-004'), reportado_por_id: adminId },
    { numavance: 3, porcentaje_avance: 100, descripcion: 'Red eléctrica modernizada al 100%. Eficiencia mejorada en 35%. Acta de recepción firmada.', fecha_presentacion: '2025-11-30', metaId: M('META-004').id, contratistaId: C('CONT-004').id, alcanceId: AL('CONT-004','META-004'), reportado_por_id: adminId },
    // META-005 CONT-011
    { numavance: 1, porcentaje_avance: 60, descripcion: 'Suministro y ensamblaje de 80 puestos ergonómicos. Divisiones modulares en planta baja.', fecha_presentacion: '2025-02-28', metaId: M('META-005').id, contratistaId: C('CONT-011').id, alcanceId: AL('CONT-011','META-005'), reportado_por_id: adminId },
    { numavance: 2, porcentaje_avance: 100, descripcion: '200 puestos equipados al 100%. 10 salas y 2 auditorios amoblados. Acta definitiva firmada.', fecha_presentacion: '2025-05-30', metaId: M('META-005').id, contratistaId: C('CONT-011').id, alcanceId: AL('CONT-011','META-005'), reportado_por_id: adminId },
  ];
